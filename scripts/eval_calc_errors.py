...
from auto_pose.bop_toolkit.bop_toolkit_lib import dataset_params
from auto_pose.bop_toolkit.bop_toolkit_lib import inout
from auto_pose.bop_toolkit.bop_toolkit_lib import misc
from auto_pose.bop_toolkit.bop_toolkit_lib import pose_error
from auto_pose.bop_toolkit.bop_toolkit_lib import renderer
...
p = {
  ...
  #'vsd_taus': list(np.arange(0.05, 0.51, 0.05, 20)),
...
def eval_calc_errors(eval_args, eval_dir, filename):
  ...
  p['vsd_taus'] = list(map(float, eval_args.get('METRIC', 'VSD_TAU').split(',')))
